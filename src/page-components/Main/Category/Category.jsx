...
const Category = () => {
  ...
  const { setSearchbarOpen, setCartOpen, user } = useAuth();
  console.log("user-data-in-category", user);
  ...
  const handleNavigate = useCallback(
    (id) => {
      ...
      setCartOpen(false);
      const ids = nanoid();
      const findTheProduct = categoryData.find((product) => product._id === id);
      console.log("find the product", findTheProduct);

      viewcontentEvent({
        _id: findTheProduct?._id || "",
        event_id: ids,

        // Convert string price to number!
        price: parseFloat(findTheProduct?.price) || 0,
        offerPrice: findTheProduct?.discountedPrice || 0,

        // Match the function parameter names
        name: findTheProduct?.name || "Unknown Product", // NOT item_name
        slug: findTheProduct?.uuId || "",
        brandName: "N/A", // Correct field name

        // Use category hierarchy properly
        category: findTheProduct?.category || "Uncategorized", // "Luxury"
        subMain: findTheProduct?.subCategory || "", // "Perfumes" - this becomes item_category2
        ...
        userId: user?.uid ?? "N/A", // Add from auth context
  ...
  const { data: categoryData = [], isLoading: isLoadingCategoryData } =
    useQuery({
      queryKey: ["category", category, queryParamsString, currentPage],
      queryFn: async () => {
        const response = await fetch(
          `${API_URL}/all_products/${category}?${queryParamsString}`,
          {
  ...
  const { data: maxPriceData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${API_URL}/products_max_price/${category}`,
        {
  ...
  const { isLoading: isLoadingCategoryDataCount } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${API_URL}/category_data_count?category=${category}&currentPage=${currentPage}`,
        {
  ...
  // react head
  useHead({
    title: `${category || "Category"} Items - AlIdaad`,
    meta: [
      {
        name: "description",
        content: `Browse ${category || "products"} at AlIdaad`,
      },
    ],
  });

  if (isLoadingCategoryData || isLoadingCategoryDataCount) return <Loading />;

  return (
    <div className="min-h-[70dvh] mt-20 md:mt-24 md:max-w-10/12 mx-auto px-4 sm:px-6 lg:px-8 border-t">
      <h1 className="text-2xl sm:text-3xl md:text-4xl mb-3 sm:mb-4 mt-2">
        {category}
      </h1>
      ...
      <div>
        <FilterSortSidebar
          isOpen={isOpen}
          setIsOpen={setIsOpen}
          productLength={categoryData?.length}
          setFilter={setFilter}
          setSort={setSort}
        ...
        <div className="flex justify-between text-gray-500 my-4 md:hidden text-sm">
          <button
            ...
            className="flex gap-2 items-center hover:text-gray-700 transition-colors"
          >
            <Settings2 size={16} />
            <span className="hidden xs:inline">Filter and sort</span>
            <span className="xs:hidden">Filters</span>
          </button>
          <p className="text-xs sm:text-sm">{categoryData?.length} products</p>
        </div>
        {/* for large */}
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none outline-none px-2 py-1 text-sm w-40 rounded-md hover:border-gray-400 focus:border-gray-500 transition-colors"
              >
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none outline-none px-2 py-1 text-sm w-32 rounded-md hover:border-gray-400 focus:border-gray-500 transition-colors"
                >
                  <option disabled selected value="">
                    Price
                  </option>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 md:gap-6 gap-2">
        {categoryData.map((product) => (
          <div key={product?._id} className="flex-shrink-0 w-auto relative">
            ...
            >
              <p className="absolute top-2 right-2 bg-white px-4 z-10 rounded-xl font-[Caluxe] shadow-2xl">
                {product?.subCategory}
              </p>
              ...
              <div className="flex flex-col md:flex-row md:gap-5 md:items-center">
                ...
                <p className="font-medium text-gray-600 mt-1 font-lexend text-sm">
                ...
                <PaginationItem key={index}>
                  {typeof page === "string" ? (
                    ...
                    <PaginationLink
                      ...
                      className={`cursor-pointer text-xs sm:text-sm h-8 w-8 sm:h-9 sm:w-9 ${
                        currentPage === page
