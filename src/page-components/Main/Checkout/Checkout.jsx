  ...
  const handlePlaceOrder = useCallback(async () => {
    ...
    const calculateTotalDiscount = (products) => {
      return products.reduce((total, item) => {
        // If you have original price vs discounted price
    ...
    purchaseEvent({
      ...
      orderTotal: formData?.products_total + formData?.deliveryCharge, // Total including delivery
      totalDiscount: calculateTotalDiscount(products), // Or set to 0 if no discount tracking

      // Map products to expected format
      products: formData.products.map((item) => ({
        productId: item?.product_id || item._id,
        slug: item?.slug || item?.product_id, // Use slug if available, fallback to product_id
        name: item?.name,

        unitPrice: item?.discountedPrice || item?.price,
        quantity: item.quantity,
        totalPrice: item?.total_price || item?.discountedPrice * item?.quantity,
        ...
        // Optional: Include discount if tracked
    ...
    <div className="mt-20 md:mt-24 md:max-w-10/12 mx-auto px-4 border-t py-4">
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-4 relative">
        ...
        <form
          ...
          className="bg-white p-4 rounded-xl shadow space-y-4 md:sticky md:top-24 md:h-[80vh]"
        >
          ...
          <textarea
            required
            name="address"
            className="border rounded w-full p-2 text-sm outline-none"
        ...
        <div className="bg-[#F5F5F5] p-4 mx-3 rounded-xl space-y-4 flex flex-col">
          ...
          {!id &&
            cartProducts.map((item) => (
              <div
                key={item?._id}
                className="border rounded-lg p-3 flex gap-3 relative bg-white"
              >
                <div className="relative">
                  ...
                  />
                  <span className="absolute -top-2 -left-2 bg-black text-white text-xs px-1 rounded-full ">
                    {item?.quantity}
                  </span>
                </div>
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex items-center gap-2 justify-between">
                    <div className="flex items-center gap-2">
                      <span
                        className={`size-4 rounded-full shadow ${
                          item?.color === "white" && "border border-gray-300"
                        }`}
                        style={{ background: item?.color }}
                      />
                      <h3 className="text-justify text-gray-500 text-sm font-medium">
                        {item?.size}
                      </h3>
                    ...
                    <p className="text-sm font-semibold">
                      ...
                      <span className="font-mina text-lg">৳</span>
                    </p>
                  </div>
                </div>
          ...
          {id && (
            <div className="border rounded-lg p-3 flex gap-3 relative bg-white">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-sm">
                    ৳ {Math.ceil(product?.discountedPrice)}
                  </span>
                </div>
                ...
                <div className="mt-2 flex items-center gap-2 justify-between">
                  <div className="flex items-center gap-2">
                    <span
                      className={`size-4 rounded-full shadow ${
                        color === "white" && "border border-red-500"
                      }`}
                      style={{ background: color }}
                    />
                    <h3 className="text-justify text-black text-sm font-medium">
                      {sizes}
                    </h3>
                    <p className="text-sm">Quantity: {quantity}</p>
                  </div>
                  <p className="text-sm font-semibold">
                    {Math.ceil(product?.discountedPrice * quantity)}{" "}
                    <span className="font-mina text-lg">৳</span>
                  </p>
                </div>
              </div>
          ...
          <div className="bg-white p-4 rounded shadow space-y-2 text-sm">
            <div className="flex justify-between font-medium pt-2">
              ...
              {!id && (
                <span className="font-mina">৳ {Math.ceil(cartTotal)}</span>
              )}
              {id && (
                <span className="font-mina">
                  ৳ {Math.ceil(product?.discountedPrice * quantity)}
                </span>
              )}
            </div>
            ...
            <div className="flex justify-between font-semibold border-t pt-2 text-base">
              ...
              {id && (
                <span className="font-mina">৳{Math.ceil(singleTotal)}</span>
              )}
            </div>
            <div className="my-4">
              <button
