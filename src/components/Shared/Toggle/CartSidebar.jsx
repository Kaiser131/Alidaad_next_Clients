...
const CartSidebar = ({ isOpen, setIsOpen, data, refetch, style }) => {
  ...
  const handleUpdate = async (action, item, id) => {
    ...
    if (action === "increase") {
      try {
        ...
        const response = await fetch(
          `${API_URL}/product_details/${item.product_id}`,
          {
    ...
    const updateData = {
      ...
      total_price:
        action === "increase"
          ? item.total_price + item.discountedPrice
          : item.total_price - item.discountedPrice,
    };
  ...
  const handleBuyNow = () => {
    ...
    initiateCheckoutEvent({
      ...
      // User data - get from first cart item or user context
      userId: user?.id || user?._id,
      userName: user?.name,
      email: data[0]?.email || user?.email || "guest",

      // Map cart items to GTM format
      items: data?.map((cartItem) => ({
        // Product identification
        item_id: cartItem?.product_id,
        item_name: cartItem?.name,
        item_brand: "Alidaad", // Add your brand name
        item_category: cartItem?.category,
        item_variant: `${cartItem?.size} / ${cartItem?.color}`,

        // Pricing (per unit, not total)
        price: cartItem?.discountedPrice,
        quantity: cartItem?.quantity,

        // Optional but helpful
        item_category2: cartItem?.category,
      })),
    });
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {data?.map((item) => (
          <div key={item?._id} className="flex gap-3 relative">
            <img
              onClick={() => productNavigate(item?.product_id)}
              src={item?.image}
              alt="Product"
              className="cursor-pointer size-16 md:size-24 object-cover rounded"
            />
            <div className="flex-1">
              <div className="flex justify-between text-justify gap-4">
                <div className="w-3/4">
                  <h3
                    onClick={() => productNavigate(item?.product_id)}
                    className="cursor-pointer font-medium text-gray-800 text-justify"
                  >
                    {item?.name.split(" ").slice(0, 5).join(" ")}
                  </h3>
                  <h3 className="text-justify text-gray-500 text-sm">
                    Tk {Math.ceil(item?.discountedPrice)}
                  </h3>
                </div>
                <div className="w-1/4 flex justify-center mx-auto">
                  <h3 className="text-justify text-sm pt-1 ">
                    Tk {Math.ceil(item?.total_price)}
                  </h3>
                </div>
              </div>
              ...
              <div className="flex gap-5 items-center mt-2 justify-between">
                ...
                <span
                  className={`size-5 rounded-full shadow border ${
                    item?.color === "white" && "border-gray-300"
                  }`}
      ...
      <div className="border-t p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-lg font-bold">
            <span className="text-xl font-mina">৳</span> {Math.ceil(total)}
          </p>
        </div>
        <button
