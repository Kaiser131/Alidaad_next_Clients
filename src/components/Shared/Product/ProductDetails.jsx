...
const ProductDetails = () => {
  const { color, setColor, quantity, setQuantity, sizes, setSizes, user } =
    useAuth();
    ...
    useQuery({
      ...
      queryFn: async () => {
        const response = await fetch(
          `${API_URL}/related_products/${id}?category=${product?.category}`,
          {
  ...
  const { data: purchaseData = [], purchaseLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${API_URL}/purchased_info/${user?.email}?product_id=${id}`,
        {
  ...
  const handleAddToCart = async (product) => {
    ...
    addToCartEvent({
      ...
      // User data (required by function)
      userId: user?.uid,
      userName: user?.displayName,
      email: user?.email || "guest",

      // Price fields - UNIT price, not total!
      unitPrice: parseFloat(cartItem.discountedPrice) || 0,

      // Product info - match function parameter names
      productId: cartItem.product_id,
      slug: product?.uuId || product?.slug, // Add slug if available
      name: cartItem.name, // NOT item_name
      brandName: product?.brand || "N/A", // NOT item_brand
      ...
      variant: {
        ...
        price: parseFloat(cartItem.discountedPrice) || 0,
      },
    });
  ...
  const handleBuyNowNavigate = useCallback(
    (id) => {
      setCartOpen(false);
      const ids = nanoid();
      initiateCheckoutEvent({
        ...
        // Items array - format as CartItem[]
        items: [
          {
            // Product identification
            productId: product?._id,
            slug: product?.uuId || product?._id,
            name: product?.name ?? "N/A",
            brandName: product?.brand || "Alidaad", // Add brand if available
  ...
  const handleReview = useCallback(async () => {
    ...
    if (!reviewText)
      return toast.error(
        "Please Write Something About This Product To Submit A Review !"
      );
    try {
      setLaoding(true);
  ...
  useHead({
    ...
    meta: [{ name: "Alidaad", content: `Product-${product?.name}` }],
  });

  if (isLoading || isLoadingRelatedProducts || purchaseLoading)
    return <Loading />;
      ...
      <div className="grid lg:grid-cols-2 md:gap-8 gap-2 relative mt-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-2 md:text-lg">
            ...
            {product?.quantity && (
              <span className="bg-green-100 text-green-800 px-2 py-0.5 rounded text-sm capitalize">
          ...
          <div>
            {product?.sizes?.length !== 0 && (
              <p className="block mb-1 capitalize font-medium">
                Variants :<span className="font-normal"> {sizes}</span>
              </p>
            )}
          ...
          <div className="border rounded">
            ...
            {openShipping && (
              <div className="p-4 text-sm text-gray-700 space-y-3 text-justify">
                <p>
                  আমরা সারা বাংলাদেশে ক্যাশ অন ডেলিভারির মাধ্যমে প্রোডাক্ট
                  পাঠাই। কিছু ক্ষেত্রে, কাস্টমারের রিভিউ এর উপর নির্ভর করে
                  অগ্রিম ডেলিভারি চার্জ নেয়া হয়।
                </p>
                <p>
                  আপনাকে পার্সেল গন্তব্যে পৌঁছানোর চেক করার পারমিশন দিচ্ছি, যাতে
                  আপনি নিশ্চিত হতে পারেন যে প্রোডাক্ট সঠিক পেয়েছেন। তবে,
                  প্রোডাক্ট ব্যবহার করা যাবে না।
                </p>
                <p>
                ...
                <p>
                  নোট: অর্ডার দেয়ার পর প্রোডাক্টের বিস্তারিত, ফটো এবং ভিডিও
                  দেখুন।
                </p>
                <p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-3 items-start flex-wrap">
                    <label className="group w-24 h-24 flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 cursor-pointer hover:border-[#F92500] hover:bg-red-50 transition-all duration-200">
                      <span className="text-3xl text-gray-400 group-hover:text-[#F92500] transition-colors">
                        +
                      </span>
