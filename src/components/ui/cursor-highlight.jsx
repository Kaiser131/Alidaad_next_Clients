...
export function CursorHighlight({
  ...
  const getPointerAnimation = () => {
    ...
    switch (direction) {
      ...
      case "top":
        return {
          initial: { x: dimensions.width / 2, y: -startOffset, rotate: 0 },
          animate: { x: dimensions.width / 2, y: dimensions.height + endOffset, rotate: 0 },
        };
      case "bottom":
        return {
          initial: { x: dimensions.width / 2, y: dimensions.height + startOffset, rotate: 180 },
          animate: { x: dimensions.width / 2, y: -endOffset, rotate: 180 },
        };
      default:
        return {
          initial: { x: -startOffset, y: yPos, rotate: 275 },
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative inline-block w-full sm:w-auto", containerClassName)}>
      ...
      <div className="relative z-40">
        <span className={cn(defaultTextColors, "relative z-20 break-words", className)}>
          {content}
        </span>
        ...
        <motion.span
          key={resetKey}
          className={cn(
            "absolute inset-0 bg-clip-text text-transparent z-30 break-words",
            gradientClass,
            className
          )}
      ...
      {showPointer && dimensions.width > 0 && dimensions.height > 0 && (
        <motion.div
          ...
          <Pointer className={cn("h-3 w-3 sm:h-4 sm:w-4 text-blue-500", pointerClassName)} />
        </motion.div>
      )}
    </div>
  );
